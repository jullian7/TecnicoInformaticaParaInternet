/*  A nota final de um estudante é calculada a partir de três notas atribuídas, 
 *  respectivamente, a um trabalho de laboratório, uma avaliação semestral e um exame final. 
 *  A média das três notas mencionadas obedece aos pesos a seguir: nota e peso
 *  
 *  AVM = 2, Simulado = 3, AVT = 5
 *  
 *  Faça um programa que receba as três notas, 
 *  calcule e mostre a média ponderada e o conceito seguindo as orientações abaixo
 *  		conceitos 
 *  	A = 8,0 --- 10,0
 *  	B = 7,0 --- 8,0
 *  	C = 6,0 --- 7,0
 *  	D = 5,0 --- 6,0
 *  	E = 0,0 --- 5,0
*/

programa
{
	
	funcao inicio()
	{
		real nota1, nota2, nota3, md
		
		

               escreva("Digite a nota 1: ")
               leia(nota1)

               escreva("Digite a nota 2: ")
               leia(nota2)

               escreva(" Digite a nota 3: ")
               leia(nota3)

               md = ((nota1*2) + (nota2*3) + (nota3*5)) / 10
          
               escreva(md)

               se(md >= 8.0 ou md <= 10.0){
                     escreva("Conceito A, sua media final:" ,md)
               
               }senao se(md >= 7.0 e md <= 8.0){
                     escreva("Conceito B, a sua media final:" ,md)
               
               }senao se(md >= 6.0 e md <= 7.0){
                     escreva("Conceito C, a sua media final:" ,md)
               
               }senao se(md >= 5.0 e md <= 6.0){
                     escreva("Conceito D, a sua media final:" ,md)

               }senao se(md >= 0.0 e md <= 5.0){
                     escreva("Conceito E, a sua media final:" ,md)
               	
               }
             


        
         
	}
}
/* $$$ Portugol Studio $$$ 
 * 
 * Esta seção do arquivo guarda informações do Portugol Studio.
 * Você pode apagá-la se estiver utilizando outro editor.
 * 
 * @POSICAO-CURSOR = 975; 
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = ;
 * @FILTRO-ARVORE-TIPOS-DE-DADO = inteiro, real, logico, cadeia, caracter, vazio;
 * @FILTRO-ARVORE-TIPOS-DE-SIMBOLO = variavel, vetor, matriz, funcao;
 */